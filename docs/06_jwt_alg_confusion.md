# 06. 攻撃3: RS256/HS256混同攻撃

`stage4`を止め、`stage5-jwt-alg-confusion`を起動する。

```
go run ./cmd/stage5-jwt-alg-confusion
```
...
`cmd/stage5-jwt-alg-confusion/main.go`の`verifyToken`を見る。
...
これは実在した脆弱性クラス(CVE-2016-5431など複数のJWTライブラリで報告)。根本原因は04章と同じ — **検証アルゴリズムをトークンの`alg`ヘッダから決めていること**。
...
JWTの3つの攻撃(04〜06章)はすべて同じ原因に行き着く — **検証側が、検証方法をトークン自身(攻撃者が作れる部分)に決めさせていた**。alg:noneは「検証しない」を選ばせ、鍵の使い回しは「検証に使う鍵の種類」を選ばせた。08章の対策は、この選択権をサーバ側に固定する。
